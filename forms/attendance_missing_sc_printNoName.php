...
function collectDataForScPrint($id) {
    ...
    $sql = "select sp.`date`,0 as mhour,(select m.modul_number from modul m  where m.modul_id = sp.used_modul_id) as ei,sp.used_hours as ush,sp.modul_start_hour as s,sp.modul_end_hour as ed, sp.used_modul_id as us  from schedule_plan sp where schedule_plan_data_id=" . $id;
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        // output data of each row
        while ($row = $result->fetch_assoc()) {
...
// Page header
    function Header() {
        ...
        $this->Cell(40);
        // Title
        $this->Cell(30, 10, 'Jelenléti ív - '."__________________", 0, 0);
        // Line break
        $this->Ln(20);
    }
