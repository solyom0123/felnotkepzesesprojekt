    ...
    for (var i = 0, max = schedule.getNaptar(); i < max; i++) {
        ...
        if(!tiltottnap(schedule,actdate)){
            ...
            if (hourscanuse.length>0) {
               ...
               var cur_units = searchCurUnit(schedule,moduls,hourscanuse);
            }
        }
    }
}
function searchCurUnit(schedule,moduls,hourscanuse){
    var returnArray = new Array();
    
    for (var i = 0, max = moduls.length; i < max; i++) {
            if(canUseCurUnit(moduls,hourscanuse)){
    ...
    for (var i = 0, max = modul.getVizsgak().length; i < max; i++) {
        if (day.getTipus()==1) {
            if(modul.getVizsga(i).getTipus()=="verbal"||modul.getVizsga(i).getTipus()=="writting"){
              if(day.getOra()>=modul.getVizsga(i).Oraszam&&!modul.getVizsga(i).getUsed()){
                  return true;
              }  
            }
...
function makeUnusableUtemterv_bejegyzes_ModelfromData(dataArray,schedule){
     for (var i = 0, max = dataArray.length; i < max; i++) {
            if (dataArray[i]!=""&&dataArray[i]!=" ") {
             ...
             let unusableDate= new Utemterv_bejegyzes_Model(spDateData[1],spDateData[0],0,0,0,0,0,0);
...
function makeWeekUtemterv_bejegyzes_ModelfromArray(dataArray,schedule,type){
     for (var i = 0, max = dataArray.length; i < max; i++) {
            if(dataArray[i]>0){
             let weekDate= new Utemterv_bejegyzes_Model(i+1,"",0,0,dataArray[i],type,0,0);
...
function makeDayUtemterv_bejegyzes_ModelfromData(dataArray,schedule){
     for (var i = 0, max = dataArray.length; i < max; i++) {
            if(dataArray[i]!=""&&dataArray[i]!=" "){
             let DayDate= new Utemterv_bejegyzes_Model(0,dataArray[i],0,0,0,0,0,0);
